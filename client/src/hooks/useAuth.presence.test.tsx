...
describe("useAuth refreshProfilePresence", () => {
  it("updates presence fields and re-caches on success", async () => {
    const { result } = await signedIn();
    fake.selectResults.presence = {
      data: {
    ...
    await act(async () => {
      returned = await result.current.refreshProfilePresence();
    });

    expect(result.current.profile?.current_session_id).toBe("session-9");
    expect(result.current.profile?.current_world_id).toBe("forest");
    expect(result.current.profile?.current_room).toBe("room-9");
    // The return value is what a caller uses to decide whether presence moved,
    // so it has to be the updated profile, not the previous one.
    expect((returned as { current_room: string }).current_room).toBe("room-9");
  });

  it("returns null and leaves state alone when the read fails", async () => {
    const { result } = await signedIn();
    fake.selectResults.presence = { data: null, error: new Error("offline") };
    ...
    // A failed refresh must not blank presence — that would show friends an
    // offline dot for someone who is still in a session.
    expect(result.current.profile?.current_session_id).toBeNull();
  });

  it("returns null with no profile to refresh", async () => {
    fake.sessionResult.session = null;
    const { result } = renderHook(() => useAuth());
